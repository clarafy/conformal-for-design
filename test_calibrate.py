...
class TestDistributions(TestCase):
    def test_get_uniform_train_and_design_test(self):
        ...
        np.random.seed(0)
        train_idx1 = np.random.choice(data.n, n_train, replace=True)
        Xtrain_nxp, ytrain_n1 = data.X_nxp[train_idx1], data.get_measurements(train_idx1, seed=1)
        ...
        # draw test covariate
        test_idx1 = np.random.choice(data.n, 1, p=punnorm_n / Z)
        ytest_nx1 = data.get_measurements(test_idx1, seed=2)
...
class TestConformalRidgeCovariateIntervention(TestCase):
    ...
    def test_get_insample_adaptive_scores(self):
        ...
        self.assertTrue(np.max(np.abs(scores1_n1xy - scores2_n1xy)) < 8.89e-16)
